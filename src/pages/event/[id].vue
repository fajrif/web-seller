...
const { data: dataEvent, execute: fetchEvent } = await useApiCore(`/events/${ route.params.id }`)

const eventData = computed(() => dataEvent.value)

const deleteProduct = async id => {
  try {
    await $apiCore(`/events/${route.params.id}/delete/${id}`, { method: 'DELETE' })
  ...
  <div>
    ...
    <VCard>
      <VRow v-if="eventData">
        ...
        <VCol cols="12">
          <VTable
            v-if="eventData.products.length > 0"
            class="text-no-wrap"
          >
            <thead>
              <tr>
